...
in
{
    options.services.rankserver = with types; {
        ...
        dataDir = mkOption {
            type = types.str;
            description = "Data directory with rankable elements (.png|.txt). RELATIVE path from ~.";
            default = "rankables";
        };
        port = mkOption {
            type = types.int;
    ...
    config = mkIf cfg.enable {
        ...
        systemd.services.rankserver = {
            ...
            {
                Type = "simple";
                ExecStart = "${cfg.package}/bin/rankserver --port ${builtins.toString cfg.port} --data-dir ${cfg.dataDir}";
                ReadWritePaths = [
                    "${cfg.dataDir}"
                ];
