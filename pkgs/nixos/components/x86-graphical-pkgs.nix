...
let
  ...
  version-string = "anix - ${if local-build then "Local Build" else "v${anixpkgs-version}"} - ${
...
in
{
  ...
  home.file =
    with anixpkgs.pkgData;
    (
      {
        ".config/terminator/config".source = ../res/terminator-config; # https://rigel.netlify.app/#terminal
        ".local/share/nautilus/scripts/terminal".source = (pkgs.writeShellScript "terminal" "terminator");
