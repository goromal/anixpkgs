{ lib, clangStdenv, fetchFromGitHub, defaultPython, cmake, gmp, eigen, fmt, pkg-src }:
let 
  symforce-src = pkg-src;
  skymarshal = defaultPython.pkgs.buildPythonPackage rec {
    pname = "skymarshal";
    version = "0.0.0";
    propagatedBuildInputs = with defaultPython.pkgs; [
      ply
      numpy
      six
      ];
...
spdlog_v1_10 = let staticBuild = false; in clangStdenv.mkDerivation rec {
  name = "spdlog";
  version = "1.10.0";
  src = fetchFromGitHub {
    owner = "gabime";
...
tl-optional = clangStdenv.mkDerivation rec {
  name = "optional";
  version = "1.1.0";src = fetchFromGitHub {
    owner = "TartanLlama";
    repo = "optional";
    rev = "v${version}";
...
pythonWithPkgs = defaultPython.withPackages (p: with p; [
  argh
  skymarshal
]);
in
