...
> **Status: Implemented.** This RFC describes the pre-flake workflow as motivation context. The migration to flake-based configuration and Determinate Nix has been completed; `anix-upgrade` now uses `nixos-rebuild --flake` and channels are no longer used.
