...
**LATEST RELEASE: [v8.51.0](https://github.com/goromal/anixpkgs/tree/v8.51.0)**
...
Here is a `flake.nix` file that will get you a shell with select `anixpkgs` software (version `v8.51.0`) while also giving you access to the public cache to avoid building from source on your machine:
...
```nix
{
  ...
  inputs = {
    nixpkgs.url = "github:goromal/anixpkgs?ref=refs/tags/v8.51.0";
  };
...
Here are some `shell.nix` files to access Python packages (using version `v8.51.0` of the packages):

```nix
let
  pkgs = import (builtins.fetchTarball
    "https://github.com/goromal/anixpkgs/archive/refs/tags/v8.51.0.tar.gz") {};
