...
**LATEST RELEASE: [v6.5.6](https://github.com/goromal/anixpkgs/tree/v6.5.6)**
...
Here is a `flake.nix` file that will get you a shell with select `anixpkgs` software (version `v6.5.6`) while also giving you access to the public cache to avoid building from source on your machine:
...
```nix
{
  ...
  inputs = {
    nixpkgs.url = "github:goromal/anixpkgs?ref=refs/tags/v6.5.6";
  };
...
Here are some `shell.nix` files to access Python packages (using version `v6.5.6` of the packages):

```nix
let
  pkgs = import (builtins.fetchTarball
    "https://github.com/goromal/anixpkgs/archive/refs/tags/v6.5.6.tar.gz") {};
