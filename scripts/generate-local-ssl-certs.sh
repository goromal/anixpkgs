...
if [[ -f rootCA.pem ]] || [[ -f chain.pem ]] || [[ -f key.pem ]]; then
    ...
    mkdir -p "${BACKUP_DIR}"
    [[ -f rootCA.pem ]] && mv rootCA.pem "${BACKUP_DIR}/"
    [[ -f rootCA-key.pem ]] && mv rootCA-key.pem "${BACKUP_DIR}/"
    [[ -f chain.pem ]] && mv chain.pem "${BACKUP_DIR}/"
...
if ! command -v mkcert &> /dev/null; then
    ...
    echo "  - On NixOS: nix-shell -p mkcert"
    echo "  - On macOS: brew install mkcert"
    echo "  - On Linux: https://github.com/FiloSottile/mkcert#installation"
    exit 1
fi
